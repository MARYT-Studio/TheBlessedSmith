...
import cn.mmf.blessedsmith.event.TLSSlashBladeHooks;
...
public class ItemSlashBladeTBS extends ItemSlashBlade {
	public ItemSlashBladeTBS(ToolMaterial par2EnumToolMaterial, float defaultBaseAttackModifier) {
	...
	public void addInformation(ItemStack arg0, World arg1, List arg2, ITooltipFlag arg3) {
		...
		if (BladeUtil.getInstance().getname(nbt) != null) {
			...
			arg2.add(TextFormatting.GOLD + I18n.format("blades.crafter") + ":" + TextFormatting.GRAY
					+ I18n.format("blessedsmith.name.unnamed_smith"));
		}
	...
	public EnumSet<SwordType> getSwordType(ItemStack itemStack) {
		...
		if (BlessedSmithConfig.advanced_mode && !BladeUtil.getInstance().IsBewitchedActived.get(tag)) {
	...
	public void addInformationMaxAttack(ItemStack arg0, EntityPlayer arg1, List arg2, boolean arg3) {
		if (BlessedSmithConfig.advanced_mode && BlessedSmithConfig.attack_lock_enable) {
	...
	public void doAddAttack(ItemStack stack, EntityPlayer player, ComboSequence setCombo) {
		if (BlessedSmithConfig.advanced_mode && BlessedSmithConfig.add_drive_enable)
			return;
	...
	public float getBaseAttackModifiers(NBTTagCompound tag) {
		return (float) Math.max(2, super.getBaseAttackModifiers(tag) - BlessedSmithConfig.base_attack_modifier_decrease);
	}
	...
	public void updateAttackAmplifier(EnumSet<SwordType> swordType, NBTTagCompound tag, EntityLivingBase el,
		...
		if (BlessedSmithConfig.attack_lock_enable) {
			attackAmplifier = 0;
		} else {
			...
			if (rank < 3 || swordType.contains(SwordType.Broken) || swordType.contains(SwordType.Sealed)) {
				...
				float max = (float) ((RefineBase + RepairCount.get(tag)) * BlessedSmithConfig.refine_multiplier);
