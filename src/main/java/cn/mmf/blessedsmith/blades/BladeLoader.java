...
import cn.mmf.blessedsmith.blades.compat.ItemSlashBladeCrimson;
import cn.mmf.blessedsmith.blades.compat.ItemSlashBladeEU;
import cn.mmf.blessedsmith.blades.compat.ItemSlashBladeRF;
import cn.mmf.blessedsmith.blades.compat.ItemSlashBladeVoid;
import cn.mmf.blessedsmith.blades.compat.ItemSlashBladeWind;
import cn.mmf.blessedsmith.blades.handmade.ItemSlashBladeHandmade;
import cn.mmf.blessedsmith.blades.handmade.ItemSlashBladeSayaHandmade;
import cn.mmf.blessedsmith.item.ItemLoader;
import cn.mmf.blessedsmith.item.ItemSlashBladeDetuneTBS;
import cn.mmf.blessedsmith.item.ItemSlashBladeSaya;
import cn.mmf.blessedsmith.item.ItemSlashBladeTBS;
import ic2.core.IC2;
...
public class BladeLoader {
	public static Item blade=new ItemSlashBladeDetuneTBS(ToolMaterial.IRON,6).setDestructable(false).setModel(new ResourceLocationRaw(SlashBlade.MOD_ID, "model/named/yasha/yasha.obj")).setModelTexture(new ResourceLocationRaw("slashblade","model/unnamed/texture.png")).setRepairMaterialOreDic("ingotSteel","nuggetSteel").setMaxDamage(71).setRegistryName("white_top").setUnlocalizedName("blessedsmith.white_top");
	public static Item bladeNamed= new ItemSlashBladeNamedTBS(ToolMaterial.IRON, 4.0f).setMaxDamage(40).setRegistryName("slashblade_named");
	public static Item euBlade,windBlade,voidBlade,rfblade,crimsonBlade;
	public static Item wrapper = new ItemSlashBladeSaya(ToolMaterial.WOOD).setRegistryName("wooden_saya").setUnlocalizedName("blessedsmith.wooden_saya");
	public static Item wrapper_bamboo = new ItemSlashBladeSaya(ToolMaterial.WOOD).setModelTexture(new ResourceLocationRaw("slashblade","model/bamboo_saya.png")).setRegistryName("bamboo_saya").setUnlocalizedName("blessedsmith.bamboo_saya");
	public static Item weapon,bladeWood,bladeBambooLight,bladeSilverBambooLight,
	bladeWhiteSheath,
	bladeHandmade = new ItemSlashBladeHandmade(ToolMaterial.IRON,6).setRegistryName("handmade_white").setUnlocalizedName("blessedsmith.handmade_white"),
	bladeSayaHandmade = new ItemSlashBladeSayaHandmade(ToolMaterial.WOOD).setRegistryName("handmade_saya_0").setUnlocalizedName("blessedsmith.handmade_saya_0");
    ...
    public void register() {
		...
		weapon = new ItemSlashBladeTBS(ToolMaterial.IRON, 4 + ToolMaterial.DIAMOND.getAttackDamage())
        ...
        bladeWood = new ItemSlashBladeDetuneTBS(ToolMaterial.WOOD, 4 + ToolMaterial.WOOD.getAttackDamage())
        ...
        bladeBambooLight = new ItemSlashBladeDetuneTBS(ToolMaterial.WOOD, 4 + ToolMaterial.STONE.getAttackDamage())
                .setDestructable(true)
        ...
        bladeSilverBambooLight = new ItemSlashBladeDetuneTBS(ToolMaterial.WOOD, 4 + ToolMaterial.IRON.getAttackDamage())
                .setDestructable(true)
        ...
        bladeWhiteSheath = new ItemSlashBladeDetuneTBS(ToolMaterial.IRON, 4 + ToolMaterial.IRON.getAttackDamage())
	...
	@SideOnly(Side.CLIENT)
    private void
	setSlashBladeRender(Item blade) {
    ...
    public void registerCustomItemStack(String name, ItemStack stack){
        BladeRegistry.put(new ResourceLocationRaw(BlessedSmith.MOD_ID, name),stack);
    }
	public ItemStack getCustomBlade(String bladename) {
		if(BladeRegistry.containsKey(new ResourceLocationRaw(BlessedSmith.MOD_ID,bladename))) {
			return BladeRegistry.get(new ResourceLocationRaw(BlessedSmith.MOD_ID, bladename)).copy();
        }
