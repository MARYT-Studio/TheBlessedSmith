...
import cn.mmf.blessedsmith.ClientProxy;
...
public class ItemSlashBladeRF extends ItemSlashBladeNamedTBS implements IEnergyContainerItem, IMultiModeBlade {
	...
	public ResourceLocationRaw getModelTexture(ItemStack par1ItemStack) {
		...
		ResourceLocationRaw result = ((ItemSlashBladeNamedTBS) par1ItemStack.getItem()).getModelTexture();
	...
	public ResourceLocationRaw getModelLocation(ItemStack par1ItemStack) {
		NBTTagCompound tag = getItemTagCompound(par1ItemStack);
		ResourceLocationRaw result = ((ItemSlashBladeNamedTBS) par1ItemStack.getItem()).getModel();
		if (isEmpowered(par1ItemStack)) {
	...
	public void addInformation(ItemStack stack, World arg1, List tooltip, ITooltipFlag arg3) {
		...
		if (StringUtil.getInstance().isAltKeyDown()) {
			...
			tooltip.add(I18n.format("blessedsmith.info.hold_alt_for_details"));
		}
		...
		if (StringUtil.displayShiftForDetail && !StringUtil.getInstance().isShiftKeyDown()) {
			tooltip.add(I18n.format("info.slashblade.hold_shift_for_details"));
		}
		...
		tooltip.add(StringUtil.getInstance().localize("info.slashblade.charge") + ": " + StringUtil.getInstance().getScaledNumber(getEnergyStored(stack))
				+ " / " + StringUtil.getInstance().getScaledNumber(getMaxEnergyStored(stack)) + " RF");
		tooltip.add(StringUtil.ORANGE + getEnergyPerUse(stack) + " "
				+ StringUtil.getInstance().localize("info.slashblade.tool.energy_per_use") + StringUtil.END);
	...
	public void addEmpoweredTip(IMultiModeBlade item, ItemStack stack, List<String> tooltip) {
		if (!isEmpowered(stack)) {
			tooltip.add(StringUtil.getInstance().localizeFormat("info.slashblade.tool.charge_on",
					StringUtil.getInstance().getKeyName(ClientProxy.ChangeMode.getKeyCode())));
		} else {
			tooltip.add(StringUtil.getInstance().localizeFormat("info.slashblade.tool.charge_off",
					StringUtil.getInstance().getKeyName(ClientProxy.ChangeMode.getKeyCode())));
		}
	}
}
