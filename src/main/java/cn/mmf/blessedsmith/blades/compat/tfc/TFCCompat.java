package cn.mmf.blessedsmith.blades.compat.tfc;
...
public class TFCCompat {
	...
	public static void onRecipeRegister(RegisterSlashBladeRecipeEvent event) {
		if (!Loader.isModLoaded("tfc"))
			return;
		RecipesUtil.getInstance().addRecipe(BlessedSmith.MOD_ID, "blessedsmith.handmade_red_steel",
				new ShapedOreRecipe(new ResourceLocation(SlashBlade.MOD_ID, "handmade_red_steel"),
				BladeLoader.getInstance().getCustomBlade("blessedsmith.handmade_red_steel"),
		...
		RecipesUtil.getInstance().addRecipe(BlessedSmith.MOD_ID, "blessedsmith.handmade_blue_steel",
				new ShapedOreRecipe(new ResourceLocation(SlashBlade.MOD_ID, "handmade_blue_steel"),
				BladeLoader.getInstance().getCustomBlade("blessedsmith.handmade_blue_steel"),
				new Object[] { "  W", " B ", "LSH", 'W', BladeLoader.wrapper, 'B',
	...
	private static void addAnvil(IForgeRegistry<AnvilRecipe> registry,String name,ItemStack input, ItemStack output, Metal.Tier tier, SmithingSkill.Type skillType, ForgeRule... rules) {
		registry.register(new AnvilRecipe(new ResourceLocation(BlessedSmith.MOD_ID, name), IIngredient.of(input), output, tier, skillType, rules));
	}
	@Method(modid="tfc")
	private static void addAnvil(IForgeRegistry<AnvilRecipe> registry,String name, IIngredient<ItemStack> input, ItemStack output, Metal.Tier tier, SmithingSkill.Type skillType, ForgeRule... rules) {
		registry.register(new AnvilRecipe(new ResourceLocation(BlessedSmith.MOD_ID, name), input, output, tier, skillType, rules));
	}
	@Method(modid="tfc")
	private static void addHeat(IForgeRegistry<HeatRecipe> registry, ItemStack input, ItemStack output, float temp) {
		registry.register(new HeatRecipeSimple(IIngredient.of(input), output, temp).setRegistryName(BlessedSmith.MOD_ID, input.getUnlocalizedName().substring(BlessedSmith.MOD_ID.length()+6)));
	}
	@Method(modid="tfc")
	private static void addWelding(IForgeRegistry<WeldingRecipe> registry, ItemStack input1,ItemStack input2, ItemStack output, Metal.Tier tier) {
		registry.register(new WeldingRecipe(new ResourceLocation(BlessedSmith.MOD_ID, (output.getUnlocalizedName().substring(BlessedSmith.MOD_ID.length()+6)).toLowerCase()), IIngredient.of(input1), IIngredient.of(input2), output, tier, null));
