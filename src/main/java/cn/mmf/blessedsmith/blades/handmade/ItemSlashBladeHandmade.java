package cn.mmf.blessedsmith.blades.handmade;
...
public class ItemSlashBladeHandmade extends ItemSlashBladeTBS {
	...
	public void addInformation(ItemStack arg0, World arg1, List arg2, ITooltipFlag arg3) {
		...
		if (StringUtil.getInstance().isAltKeyDown()) {
			...
			arg2.add(I18n.format("blessedsmith.info.hold_alt_for_details"));
		}
	}
    ...
    public void getSubItems(CreativeTabs tab, NonNullList<ItemStack> subItems) {
        ...
        subItems.add(item0);
    	BladeLoader.getInstance().registerCustomItemStack("blessedsmith.handmade_white", item0);
        
        if(Loader.isModLoaded("tfc")) {
            ItemStack item = new ItemStack(this);
            NBTTagCompound tag = new NBTTagCompound();
            item.setTagCompound(tag);
        	ItemSlashBladeNamed.CurrentItemName.set(tag, "blessedsmith.handmade_red_steel");
        	ItemSlashBlade.TextureName.set(tag, "handmade/white_red_steel");
        	subItems.add(item);
        	BladeLoader.getInstance().registerCustomItemStack("blessedsmith.handmade_red_steel", item);
            
        	ItemStack item1 = new ItemStack(this);
            NBTTagCompound tag1 = new NBTTagCompound();
            item1.setTagCompound(tag1);
        	ItemSlashBladeNamed.CurrentItemName.set(tag1, "blessedsmith.handmade_blue_steel");
        	ItemSlashBlade.TextureName.set(tag1, "handmade/white_blue_steel");
        	subItems.add(item1);
        	BladeLoader.getInstance().registerCustomItemStack("blessedsmith.handmade_blue_steel", item1);
        }
        ...
        if(Loader.isModLoaded("thaumcraft")) {
            ...
            NBTTagCompound tag = new NBTTagCompound();
            item.setTagCompound(tag);
        	ItemSlashBladeNamed.CurrentItemName.set(tag, "blessedsmith.handmade_thaum");
        	ItemSlashBlade.TextureName.set(tag, "handmade/white_thaum");
        	subItems.add(item);
        	BladeLoader.getInstance().registerCustomItemStack("blessedsmith.handmade_thaum", item);
            
        	ItemStack item1 = new ItemStack(this);
            NBTTagCompound tag1 = new NBTTagCompound();
            item1.setTagCompound(tag1);
        	ItemSlashBladeNamed.CurrentItemName.set(tag1, "blessedsmith.handmade_void");
        	ItemSlashBlade.TextureName.set(tag1, "handmade/white_void");
        	SpecialEffects.addEffect(item1, SELoader.SAP);
        	subItems.add(item1);
        	BladeLoader.getInstance().registerCustomItemStack("blessedsmith.handmade_void", item1);
        }
    }
